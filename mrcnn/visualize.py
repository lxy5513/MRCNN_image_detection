...
# myfont = fm.FontProperties(fname='/usr/local/share/fonts/simhei.ttf', size=25)
...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    individual add: save_path 表示保存的路径及文件名
    """
    ...
    for i in range(N):
        ...
        # Mask
        mask = masks[:, :, i]
        # mask_list = [mask[i][j] for i in range(1440) for j in range(1080) if mask[i][j]==True]
